...
@section('content')
				...
				<div class="col-lg-8">
					<nav style="--falcon-breadcrumb-divider:'»';" aria-label="breadcrumb">
						<ol class="breadcrumb">
							...
							<li class="breadcrumb-item active" aria-current="page">Quy đổi giờ chuẩn</li>
						</ol>
					</nav>
				</div>
			</div>
	...
	<div class="card mb-0">
		<div class="card-header bg-light">
			<h5 class="mb-0">Quy đổi giờ chuẩn</h5>
		</div>
		<div class="card-body pb-0">
			<p>
				...
				<a href="{{ route('supmanager.quydoigiochuan.xuat') }}" class="btn btn-success"><i class="fal fa-download"></i> Xuất ra Excel</a>
			</p>
				...
				<table id="DataList" class="table table-bordered table-hover table-sm overflow-hidden">
					<thead>
						<tr>
                            ...
                            <th class="text-nowrap" width="15%">Giờ chuẩn</th>
					...
					<tbody>
						{{-- @foreach($quydoigiochuan as $value) --}}
						@if($quydoigiochuan)
							<tr>
								{{-- <td class="align-middle">{{ $loop->iteration }}</td> --}}
								<td class="align-middle">{{ $quydoigiochuan->ID }}</td>
								<td class="align-middle">{{ $quydoigiochuan->HoatDong }}</td>
                                <td class="align-middle">{{ $quydoigiochuan->GioChuan }}</td>
                                <td class="align-middle">{{ $quydoigiochuan->NamHoc }}</td>
								<td class="align-middle text-center"><a href="#sua" data-bs-toggle="modal" data-bs-target="#myModalEdit" onclick="getCapNhat('{{ $quydoigiochuan->ID }}', '{{ $quydoigiochuan->HoatDong }}', '{{ $quydoigiochuan->GioChuan }}', '{{ $quydoigiochuan->NamHoc }}'); return false;"><i class="fal fa-edit"></i></a></td>
								<td class="align-middle text-center pe-1"><a href="#xoa" data-bs-toggle="modal" data-bs-target="#myModalDelete" onclick="getXoa('{{ $quydoigiochuan->ID }}'); return false;"><i class="fal fa-trash-alt text-danger"></i></a></td>
							</tr>
						@endif
						{{-- @endforeach --}}
					</tbody>
	...
	<form action="{{ route('supmanager.quydoigiochuan.them') }}" method="post" class="needs-validation" novalidate>
		@csrf
			...
			<div class="modal-dialog modal-dialog-centered">
				<div class="modal-content">
					<div class="modal-header">
						<h5 class="modal-title" id="myModalLabel">Thêm quy đổi giờ chuẩn</h5>
					...
					<div class="modal-body">
						...
						<div class="mb-3">
							<label class="form-label" for="GioChuan"><span class="badge bg-info">3</span> Giờ chuẩn<span class="text-danger fw-bold">*</span></label>
							<input type="number" class="form-control @error('GioChuan') is-invalid @enderror" id="GioChuan" name="GioChuan" value="{{ old('GioChuan') }}" required />
							@error('GioChuan')
								<div class="invalid-feedback"><strong>{{ $message }}</strong></div>
							@enderror
						</div>	
	...
	<form action="{{ route('supmanager.quydoigiochuan.sua') }}" method="post" class="needs-validation" novalidate>
		@csrf
		...
		<div class="modal fade" id="myModalEdit" tabindex="-1" data-bs-backdrop="static" data-bs-keyboard="false" aria-labelledby="myModalLabelEdit">
			<div class="modal-dialog modal-dialog-centered">
				<div class="modal-content">
					<div class="modal-header">
						<h5 class="modal-title" id="myModalLabelEdit">Cập nhật thông tin hệ số quy đổi</h5>
						<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
					</div>
					<div class="modal-body">
						<div class="mb-3">
						...
						<div class="mb-3">
							<label class="form-label" for="GioChuan_edit"><span class="badge bg-info">3</span> Giờ chuẩn <span class="text-danger fw-bold">*</span></label>
							<input type="text" class="form-control @error('GioChuan_edit') is-invalid @enderror" id="GioChuan_edit" name="GioChuan_edit" value="{{ old('GioChuan_edit') }}" required />
							@error('GioChuan_edit')
								<div class="invalid-feedback"><strong>{{ $message }}</strong></div>
							@enderror
						</div>
	...
	<form action="{{ route('supmanager.quydoigiochuan.xoa') }}" method="post" class="needs-validation" novalidate>
		@csrf
		...
		<div class="modal fade" id="myModalDelete" tabindex="-1" data-bs-backdrop="static" data-bs-keyboard="false" aria-labelledby="myModalLabelDelete">
			<div class="modal-dialog modal-dialog-centered">
				<div class="modal-content">
					<div class="modal-header">
						<h5 class="modal-title" id="myModalLabelDelete">Xóa quy đổi giờ chuẩn</h5>
						<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
					</div>
	...
	<form action="{{ route('supmanager.quydoigiochuan.nhap') }}" method="post" enctype="multipart/form-data" class="needs-validation" novalidate>
		@csrf
...
@section('javascript')
	<script>
		function getCapNhat(ID, HoatDong, GioChuan, NamHoc) {
			$('#id_old').val(ID);
			$('#ID_edit').val(ID);
			$('#HoatDong_edit').val(HoatDong);
			$('#GioChuan_edit').val(GioChuan);
		...
		@if($errors->has('ID') || $errors->has('HoatDong')|| $errors->has('GioChuan')|| $errors->has('NamHoc'))
			var myModalThem = new bootstrap.Modal(document.getElementById('myModalThem'));
			myModalThem.show();
		@endif
		
		@if($errors->has('ID_edit') || $errors->has('HoatDong_edit')|| $errors->has('GioChuan_edit')|| $errors->has('NamHoc_edit'))
